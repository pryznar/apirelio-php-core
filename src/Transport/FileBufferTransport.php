...
class FileBufferTransport implements EventTransport
{
    ...
    public function send(array $events): void
    {
        ...
        if (! is_dir($directory) && ! mkdir($directory, 0775, true) && ! is_dir($directory)) {
            throw new RuntimeException('Unable to create the Apirelio buffer directory.');
        }
        ...
        if (file_put_contents($this->config->path, implode("\n", $lines)."\n", FILE_APPEND | LOCK_EX) === false) {
            throw new RuntimeException('Unable to write the Apirelio event buffer.');
        }
    ...
    private function flush(int $size): void
    {
        ...
        if ($handle === false || ! flock($handle, LOCK_EX)) {
            throw new RuntimeException('Unable to lock the Apirelio event buffer.');
        }
